...
- DS_Store 管理：按需清理和监控用户明确选择的目录，可选择登录时启动 miniTools
...
DS_Store 管理默认关闭。用户在设置中选择一个或多个目录后，可以立即递归清理其中的
`.DS_Store`，也可以开启 FSEvents 监控以清理后续产生的文件。它不会默认监听或遍历整个
磁盘，并跳过废纸篓、系统目录、应用包和无法读取的目录。设置页显示本次运行成功删除的
...
- `Features/DSStoreManagement`：目录选择、FSEvents 监控、清理策略和登录启动状态
