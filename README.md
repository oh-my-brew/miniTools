...
窗口控制默认使用 `⇧⌃⌥⌘`：`U/I/J/K` 对应四个角，重复触发在半宽和三分之一宽之间切换；`H/L` 对应左/右侧，重复触发在三分之二和二分之一宽之间切换；`Y` 在上/下半屏之间切换；`O` 在右/左三分之一之间切换；`\` 铺满当前屏幕可用区域。设置中可以开启“使用 macOS 原生窗口布局”：系统菜单支持当前目标时直接调用 macOS 的布局或跨屏能力，否则自动回退到 miniTools 原有几何实现；成功后右上角会显示本次实际使用的是“macOS 原生”还是“miniTools”。该选项默认关闭。Safari 窗口切换和窗口控制共用辅助功能权限，首次使用需要在“系统设置 → 隐私与安全性 → 辅助功能”中允许 miniTools，不再需要 Safari 自动化权限。
...
“使用统计”只在本机保存成功操作的动作标识、次数和时间，不统计 Safari 窗口、DS_Store
