...
窗口控制默认使用 `⇧⌃⌥⌘`：`U/I/J/K` 对应四个角，重复触发在半宽和三分之一宽之间切换；`H/L` 对应左/右侧，重复触发在二分之一宽和三分之二宽之间切换；`Y` 在上/下半屏之间切换；`O` 在右/左三分之一之间切换；`\` 铺满当前屏幕可用区域。切换到另一个区域时会重新从二分之一宽开始，不沿用上一个区域的循环进度。设置中可以开启“使用 macOS 原生窗口布局”：系统菜单支持当前目标时直接调用 macOS 的布局或跨屏能力，否则自动回退到 miniTools 原有几何实现；只有确实调用到 macOS 原生能力时，右上角才提示“macOS 原生 · 目标尺寸”。该选项默认关闭。Safari 窗口切换和窗口控制共用辅助功能权限，首次使用需要在“系统设置 → 隐私与安全性 → 辅助功能”中允许 miniTools，不再需要 Safari 自动化权限。
