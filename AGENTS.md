...
- “使用 macOS 原生窗口布局”默认关闭；开启后通过目标应用的 AX 菜单项直接调用 macOS 布局和跨屏动作，不模拟系统快捷键。找不到、不可用或不匹配的系统动作必须回退到现有几何实现，并在成功后于屏幕右上角提示实际使用“macOS 原生”或“miniTools”。
...
- 仅统计编码与转换、窗口管理、鼠标跨屏三类。Safari 窗口、DS_Store 管理、合盖运行、失败动作和自动事件不得计数。
