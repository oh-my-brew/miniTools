...
- “优先使用系统窗口操作”默认关闭；开启后通过目标应用的 AX 菜单项直接调用 macOS 布局和跨屏动作，不模拟系统快捷键。找不到、不可用或不匹配的系统动作必须回退到现有几何实现。
...
- 状态栏通过单个“启动合盖运行”或“关闭合盖运行”菜单项直接切换状态，不提供时长选项。
...
- 当前会话和最近 5 次关闭记录需要持久化；关闭记录包含时间与原因，不再记录运行时长或运行方式。
